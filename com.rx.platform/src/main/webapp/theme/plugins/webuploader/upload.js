jQuery(function() {
	...
	// 缩略图大小
	thumbnailWidth = 210 * ratio, thumbnailHeight = 40 * ratio,
	...
	if (!WebUploader.Uploader.support()) {
		var msg = '附件上传功能不支持您的浏览器！如果你使用的是IE浏览器，请尝试升级 flash 播放器';
		layui.layer.msg(msg, {
			icon : 5,
			time : 1000
		});
	...
	uploader = WebUploader.create({
		...
		accept : {
			title : '文件',
			extensions : 'bmp,jpg,png,tiff,gif,pcx,tga,exif,fpx,svg,psd,cdr,pcd,dxf,ufo,eps,ai,raw,rar,zip,tar,doc,docx,xls,xlsx,ppt,pptx,pdf,txt',// png,jpg,jpeg,gif,bmp,rar,zip,tar,doc,docx,xls,xlsx,ppt,pptx,pdf,txt
			mimeTypes : '*/*'
		},

		// swf文件路径 
		swf : '/theme/plugins/webuploader/Uploader.swf',

		disableGlobalDnd : true,
		
		formData:{fkid:$("#fkid").val()},

		chunked : true,
		server : '/file/mult_upload',
		fileNumLimit : 10,
	...
	function addFile(file) {
		...
		$btns = $('<div class="file-panel">' + '<span class="cancel">删除</span>' + '<span class="rotateRight">向右旋转</span>' + '<span class="rotateLeft">向左旋转</span></div>').appendTo($li), $prgress = $li
