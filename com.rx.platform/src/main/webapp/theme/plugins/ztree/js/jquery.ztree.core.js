...
(function ($) {
    var settings = {}, roots = {}, caches = {},
        ...
        view = {
            ...
            makeNodeIcoClass: function (setting, node) {  // zhogndd 
                var icoCss = ["ico"];
                var isIcon =false;
                if (!node.isAjaxing) {
                	//zhongdd
                    ...
                    //console.log(icon);
                    if(typeof(icon) != "undefined"&&icon.indexOf("icon-") != -1){//zhongdd
                    	icoCss = ["iconfont "+icon];
                    	isIcon =true;
                    }else{
                    	icoCss[0] = (node.iconSkin ? node.iconSkin + "_" : "") + icoCss[0];
                    	if (node.isParent) {
            ...
            makeNodeIcoStyle: function (setting, node) {
                ...
                if (!node.isAjaxing) {
                    var icon = (node.isParent && node.iconOpen && node.iconClose) ? (node.open ? node.iconOpen : node.iconClose) : node[setting.data.key.icon];
                    //if (icon) icoStyle.push("background:url(", icon, ") 0 0 no-repeat;");//zhongdd
