  layui.config({
    ...
    table.on('tool(Layui-table-toolbar)', function(obj){
      var data = obj.data;
      if(obj.event === 'detail'){
          opendgviewLook(data.id_table_name);
        } else if(obj.event === 'del'){
        	deleteById(data.id_table_name)
        } else if(obj.event === 'edit'){
        	opendgviewEdit(data.id_table_name)
        }
    });
  ...
  //查看页面
    ...
    function opendgviewEdit(id){
    	var index = layer.open({
            type: 2,
            title:'在线填报-数据表管理',
            content: link2EditS+'?id_table_name='+id,
            area: ['300px', '300px'],
            maxmin: true,
            end: function(index, layero){ 
          	    layer.close(index)
