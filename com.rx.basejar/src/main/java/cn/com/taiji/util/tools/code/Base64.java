 ...
 * 
 * @ClassName:  Base64   
 * @Description:TODO Base64编码服务类
 * @author: zhongdd
 * @date:   2018年5月19日 上午8:17:03   
 *     
 *
 */
public final class Base64 {
	...
	public static byte[] encode(byte binaryData[]) {
		...
		for (i = 0; i < numberTriplets; i++) {
			...
			byte val3 = (b3 & -128) != 0 ? (byte) (b3 >> 6 ^ 252) : (byte) (b3 >> 6);
	 ...
	 * @throws IOException
	 */
	public static long encode(InputStream inputStream, OutputStream outputStream) throws IOException {
		long totalSize = 0L;
		...
		label0: do {
			...
			if (size < 3) {
				...
				if (numRemainder != 0) {
					encodeSize = (size / 3) * 3;
					pushbackInputStream.unread(binaryData, encodeSize, numRemainder);
				}
			}
	 ...
	 * @throws IOException
	 */
	public static long decode(InputStream inputStream, OutputStream outputStream) throws IOException {
		long totalSize = 0L;
		...
		label0: do {
			...
			if (size < 4) {
				...
				if (numRemainder != 0) {
					decodeSize = (size / 4) * 4;
					pushbackInputStream.unread(base64Data, decodeSize, numRemainder);
				}
			}
