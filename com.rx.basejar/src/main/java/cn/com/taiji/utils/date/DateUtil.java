...
public class DateUtil {
	...
	private static String deflateChinesePattern = "yyyy年MM月dd日";//transient
...
////		System.out.println(formater.toLocalizedPattern());
		...
		System.out.println("当前日期根据格式转换："+getNowTime("[YY年MM月dd日]"));
		...
		System.out.println("根据数字获得对应的字符用于日期："+getNumberString("2018年12月29日"));
	...
	/*	System.out.println("获得上月第一天"+Date2String(getPreviousMonthFirst(),2));
		...
		System.out.println(strDate2ChineseDate("2018年11月06日"));
		System.out.println(strDate2ChineseDate("2018-11-06"));
...
/**************************************日期转换方法**************************************/
	...
	/**
	 * 时间转换为大写
	 * @param strDate  2018-11-06
	 * @return 二零一八年一二月二9日
	 */
	public static String strDate2ChineseDate(String strDate){
        ...
        System.out.println( new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z",Locale.CHINA).format(new Date())); 
		SimpleDateFormat sf = new SimpleDateFormat(deflatePattern);
		try {
			Calendar c = Calendar.getInstance();
			c.setTime(sf.parse(strDate));
			
			
			
			
			int index_ = 0 ;
			for (String string : chineseDateArray) {
				strDate = strDate.replace( String.valueOf(index_), string);
				index_ ++ ;
			}
