    ...
    table.render({
      ...
      ,cols: [[
       // {type: 'checkbox', fixed: 'left'},
        ...
        ,{width:300,field:'imp_model_name', title:'数据导入文件名称', fixed: 'left',sort:true }
        ,{width:150,field:'imp_data_num', title:'导入数据量',sort:true    } 
        ,{width:150,field:'imp_version', title:'模板版本',sort:true  }
        ,{field:'imp_table_name', title:'数据表名',sort:true  }
        ,{width:150,field:'imp_status', title:'状态',sort:true ,templet:function(res){
        	if(res.imp_status=='0'){
        		return '未校验';
        	}else if(res.imp_status=='1'){
        		return '校验通过';
        	}else if(res.imp_status=='2'){
        		return '校验不合格';
        	}else if(res.imp_status=='3'){
        		return '已录入';
        	}
        } }
        ,{fixed: 'right', title:'操作', fixed: 'right',width:300, align: 'center', toolbar: '#Layui-table-toolbar-barDemo'}
      ]]
      ,limit:10
...
//          	    }
    ...
    //编辑页面
    function opendgviewPush(id){
    	var index = layer.open({
            type: 2,
            title:'在线填报-数据导入',
            content: linkPushS+'?imput_id='+id,
            area: ['300px', '300px'],
            maxmin: true,
            end: function(index, layero){ 
          	    layer.close(index)
