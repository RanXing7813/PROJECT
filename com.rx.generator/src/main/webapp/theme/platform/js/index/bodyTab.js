...
layui.define([ "element", "jquery" ], function(exports) {
		...
		if (_this.attr("target") == "_blank") {
			...
			if (_this.find("i.iconfont,i.layui-icon").attr("data-icon") != undefined) {
				if (_this.find("i.iconfont").attr("data-icon") != undefined) {
					title += '<i class="iconfont ' + _this.find("i.iconfont").attr("data-icon") + '">' + _this.find("i.iconfont").html() + '</i>';
				} else {
					title += '<i class="layui-icon">' + _this.find("i.layui-icon").html() + '</i>';
				}
			}
			// 已打开的窗口中不存在
			if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
				...
				title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + tabIdIndex + '"><img src="theme/platform/css/icons/cross.png" /></i>';

				var layId = new Date().getTime();

				element.tabAdd(tabFilter, {
					title : title,
					content : "<iframe id='f_" + layId + "' src='" + _this.attr("data-url") + "' data-id='" + tabIdIndex + "'></frame>",
					id : new Date().getTime()
				})
				// 当前窗口内容
				var curmenu = {
				...
				// 点击左边菜单 刷新iframe页面
				var fid = "f_" + that.getLayId(_this.find("cite").text());
				$('#' + fid).attr('src', $('#' + fid).attr('src'));
			}
		}
	}
	...
	Tab.prototype.tabMove = function() {
		$(window).on(
				"resize",
				function() {
					var topTabsBox = $("#top_tabs_box"), topTabsBoxWidth = $("#top_tabs_box").width(), topTabs = $("#top_tabs"), topTabsWidth = $("#top_tabs").width(), tabLi = topTabs
							.find("li.layui-this"), top_tabs = document.getElementById("top_tabs");
					;
